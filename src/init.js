		...
		availableSettings: {
			...
			trovo: {
				...
				timestampFormat: {
					...
					type: 'select',
					items: [
						{ value: 12, label: '01:33:37 (12-hour clock)' }, {
							value: 24,
...
const YouTube = {
	...
	handleMessage(node) {
		...
		if (settings.youtube.timestampFormat.toString() === '24' && timestamp) {
			...
			if (timestamp.textContent.toLowerCase().includes('am') && split[0] == 12) {
				split[0] = (parseInt(split[0]) - 12).toString();
				timestampText = split.join(':');
			}
	...
	init() {
		...
		const init = (documentElement, target) => {
			...
			for (const element of this.document.querySelectorAll('yt-live-chat-text-message-renderer')) {
				this.handleMessage(element);
			}

			chatMessageObserver.observe(target, { attributes: true, childList: true, characterData: true });
			...
			const setupSettingOption = () => {
				...
				settingOption.addEventListener('click', () => {
					...
					this.document.body.click(); // click on body to close the settings container
				});
				const popupRenderer = node.querySelector('ytd-menu-popup-renderer');
				popupRenderer.style.removeProperty('max-height');
			};

			if (this.style && this.style.isConnected === false) {
				this.style = null;
			}

			const settingObserver = new MutationObserver((mutations) => {
				for (const mutation of mutations) {
					for (const node of mutation.addedNodes) {
						const nodeName = node.nodeName.toLowerCase();
						if ((nodeName === 'iron-dropdown' || nodeName === 'tp-yt-iron-dropdown') && node.classList.contains('yt-live-chat-app')) {
							// youtube remove all html elements and add them again, observe the attributes for the dropdown node
							// if aria-hidden old value is "true" then add the menu item
							const toggleDropdownObserver = new MutationObserver((mutations) => {
								for (const mutation of mutations) {
									...
									if (mutation.oldValue === 'true') {
										// i guess youtube use something like a virtual dom and need time to render the dom
										// we need the timeout to add the item before youtube remove all old html elements
										window.setTimeout(setupSettingOption, 1);
									}

									break;
								}
							});
							toggleDropdownObserver.observe(node, {
								attributes: true,
